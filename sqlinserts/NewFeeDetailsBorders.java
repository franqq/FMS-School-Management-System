 ...
 * @author franq
 */
public class NewFeeDetailsBorders {
     private static DBConnection dbconn;
     
     private static void createNewFeeDetails_Borders(int classid,int termid,double lostitem,double residence,double meals, double total) throws SQLException
     {
          PreparedStatement stmt = null;
        dbconn = new DBConnection();
        Connection conn = dbconn.vconnection;
        String insert = "INSERT INTO `fee_details_borders`(`classID`, `termID`, `Lost School Item Fee`, `Residence Fee`, `Meals Fee`, `Total`)"
                + "VALUES(?,?,?,?,?,?)";
        try {
      ...
      public static void main(String args[]) {
          try {
            createNewFeeDetails_Borders(1, 1, 455.00, 3500.00, 3455.55, 0);
          } catch (SQLException ex) {
