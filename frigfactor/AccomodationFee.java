 ...
 * @author franq
 */
public class AccomodationFee {
    public static Double getAccomodationFee(int residenceid)
    {
        DBConnection dbconn =new DBConnection();
        Connection conn = dbconn.vconnection;
        PreparedStatement pstmt;
        Double schoolname=0.00;
        String sql = "SELECT `Fee` FROM `residence` WHERE `ID`=?";
        try {
